...
def rectLucasKanadePrevious( video_frames ):
    rect_arr_prev = []
    rect_arr_prev.append(rect)

    rect_new = rect.copy()

    template_img = get_template_from_img(video_frames[:, :, 0], rect_new)
...
if __name__ == '__main__':
    ...
    for i in range(1, video_frames.shape[2]):
        print('-------------------------------------  ', i, '  -------------------------------------------')
        p = LucasKanadeBasis( template_img, video_frames[:,:,i], rect_new, bases, np.zeros(2) )
        print('p: ', p)
        rect_new =  rect_new + np.asarray([ p[0], p[1], p[0], p[1]])
    ...
    # print('rect_arr: ', rect_arr)
    print('rect_arr shape: ', rect_arr.shape)

    np.save('../data/sylvseqrects.npy', rect_arr)

    ##################################  RUN PREVIOUS LUCAS KANADE  ###################################
    rect_arr_prev = rectLucasKanadePrevious( video_frames )

    frame_list = [1, 200, 300, 350, 400]
    ...
    for i in range(len(frame_list)):
        ...
        rect_prev = patches.Rectangle(rect_arr_prev[frame_list[i], :2], rect_arr_prev[frame_list[i], 2] - rect_arr_prev[frame_list[i], 0], rect_arr_prev[frame_list[i], 3] - rect_arr_prev[frame_list[i], 1], linewidth=0.6, edgecolor='g', fill=False)
        axarr[i].add_patch(rect_prev)

        axarr[i].axis('off')

    plt.subplots_adjust(wspace=0.025, hspace=0)

    plt.savefig('../lbahl/2_3.png', bbox_inches='tight',  dpi = 1000)