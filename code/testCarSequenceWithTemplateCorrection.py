...
if __name__ == '__main__':
    ...
    template_img_first = template_img.copy()

    for i in range(1, video_frames.shape[2]):
        print('-------------------------------------  ', i, '  -------------------------------------------')
        p = LucasKanade( template_img, video_frames[:,:,i], rect_new, np.zeros(2) )
        print('p: ', p)
        p_star =  LucasKanade( template_img_first, video_frames[:,:,i], rect_new, p )
        print('p_star: ', p_star)
        rect_new = rect_new + np.asarray([ p_star[0], p_star[1], p_star[0], p_star[1]])
    ...
    np.save('../data/carseqrects-wcrt.npy', rect_arr)

    rect_arr_prev = np.load('../data/carseqrects.npy')
    ...
    plt.savefig('../lbahl/1_4.png', bbox_inches='tight',  dpi = 1000)