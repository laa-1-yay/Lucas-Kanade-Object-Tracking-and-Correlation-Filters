...
def animate(i):
    ...
    if i < N:  # If the animation is still running
        ...
        print('X: ', X.shape)  # 1305 x 1305
        print('Y: ', Y.shape)  # 1305 x 1

        #####################################  CASE 1  ###########################################
        lamda = 1
        S = np.matmul(X, X.T)
        temp = S + np.multiply(lamda, np.eye(X.shape[0], X.shape[0]))
        temp_inv = np.linalg.inv(temp)
        g = temp_inv.dot(X).dot(Y)
        print('g shape: ', g.shape)
        g_reshaped = g.reshape(dsize)
        print('g_reshaped shape: ', g_reshaped.shape)
        print('g_reshaped: ', g_reshaped)

        fig3 =  plt.figure()
        plt.imshow(g_reshaped)
        plt.title('g_reshaped for lambda = 1')

        correlate_output = correlate(img, g_reshaped)
        convolve_output = convolve(img, g_reshaped)

        print('correlate_output for lambda = 1: ', correlate_output)
        print('convolve_output for lambda = 1: ', convolve_output)

        fig4 =  plt.figure()
        plt.imshow(correlate_output)
        plt.title('correlate_output for lambda = 1')

        fig5 =  plt.figure()
        plt.imshow(convolve_output)
        plt.title('convolve_output for lambda = 1')
        ...
        fig6 =  plt.figure()
        plt.imshow(g_reshaped)
        plt.title('g_reshaped for lambda = 0')

        correlate_output = correlate(img, g_reshaped)
        convolve_output = convolve(img, g_reshaped)

        print('correlate_output for lambda = 0: ', correlate_output)
        print('convolve_output for lambda = 0: ', convolve_output)

        fig7 =  plt.figure()
        plt.imshow(correlate_output)
        plt.title('correlate_output for lambda = 0')

        fig8 =  plt.figure()
        plt.imshow(convolve_output)
        plt.title('convolve_output for lambda = 0')
